...
import com.wdtx.littlebell.model.HeartRate;
...
public class HeartRateActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        getSupportActionBar().setTitle("心率监控");
        ...
        spinner.setOnItemSelectedListener(new AdapterView.OnItemSelectedListener() {
            ...
            public void onItemSelected(AdapterView<?> parent, View view, int position, long id) {
                ...
                VolleyUtil.getInstance().requestPost("/heartrate", map,
                        new VolleyCallBack<List<HeartRate>>() {
                            @Override
                            public void onSuccess(List<HeartRate> data) {
                                generateData(data);
                            }

                            @Override
                            public void onFail(int code, String msg) {
                                Toast.makeText(HeartRateActivity.this, msg, Toast
                                        .LENGTH_SHORT)
                                        .show();
                            }
                            ...
                            public void onError(VolleyError error) {
                                ...
                                Toast.makeText(HeartRateActivity.this, "网络异常", Toast.LENGTH_SHORT)
                                        .show();
                            }
                        });
            }
    ...
    private void generateData(List<HeartRate> heartRates) {
        List<AxisValue> axisValues = new ArrayList<AxisValue>();
        List<PointValue> values = new ArrayList<>();
        for (int j = 0; j < heartRates.size(); ++j) {
            PointValue pointValue =
                    new PointValue(j, (float) heartRates.get(j).getHeartRate());
            pointValue.setLabel(heartRates.get(j).getHeartRate()+"");
            values.add(pointValue);
            axisValues.add(new AxisValue(j).setLabel(sdf.format(heartRates.get
                    (j).getTime())));
        }
        ...
        currentViewport.top = 120f;
        currentViewport.bottom = 40f;
